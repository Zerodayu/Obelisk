...
- **Role → feature gates are centralized in `lib/role-access.ts`** (`FEATURE_ACCESS`) — class-record capture (all `/ingest/*` endpoints: `faculty`/`program_chair`/`system_admin`), archive (`vpaa`/`system_admin`), PLO management (`dean` only — **no** admin bypass), role requests (`system_admin`), cluster confirm (`aqau`/`system_admin`), AI insight generation (`vpaa`/`system_admin` — guards `POST /ai/recommendation/generate`, the python-server LLM call). Changing role access = edit that file **and** its frontend mirror `frontend/lib/role-access.ts` in the same change — `test/unit/role-access-sync.test.ts` fails CI on drift.
