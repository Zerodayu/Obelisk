  ...
  └─apiRoutesV1 (prefix api/v1)
       └─ feature plugin (prefix, tags)
            ...
            └─ controller ─ service (business logic) ─ Prisma (Neon)
                              └─> python-server /analytics/* (sync, via lib/ingest/ingest-client — rollup F15, AI institutional-summary)
```
...
| `vpaa` | institution-wide | Approve CAPA/budget, **archive approved submissions + browse `/archives`**, **generate AI insights** (`POST /ai/recommendation/generate`), institutional decisions |
| `system_admin` | everything | Admin/roles, **confirm role requests**, **confirm graduation-cluster compile**, **generate AI insights**, workflow override (bypasses preparer/step/archive gates) |
...
| POST | `/api/v1/plan/plos` | `auth: true` + **dean** | Create a PLO — **dean-only** (`PloForbiddenError` → 403); code unique per program, `targetAttainmentPct` default 70 with the **≥70% hard floor** enforced (`TargetBelowFloorError`); **audited** `plo.created`. |
| PUT | `/api/v1/plan/plos/:id` | `auth: true` + **dean** | Patch a PLO's code/description/target — **dean-only**; uniqueness + floor re-checked on patched values; **audited** `plo.updated`. |
| DELETE | `/api/v1/plan/plos/:id` | `auth: true` + **dean** | Delete an unused PLO — **dean-only**; **blocked with 409** (`PloInUseError`) while mapped to a CLO or carrying `PloAttainment`/`GapRow`/`CqiEntry` history; **audited** `plo.deleted`. |
...
| GET | `/api/v1/ai/recommendation/latest` | `auth: true` | Newest persisted `ai_recommendation` row across all terms (markdown text + worst-CLO rows + term/period), or `null` body when none. Intentionally **not** `cached()` so a generate → read round-trip is always fresh |
| POST | `/api/v1/ai/recommendation/generate` | `auth: true` + **role `vpaa`/`system_admin`** (`assertCanGenerateAiInsights`) | Generate + persist an AI CQI recommendation: rollup snapshot → python-server `POST /analytics/institutional-summary` (LLM via prompt cache) → worst-CLO computation → `ai_recommendation` row. Errors: 403 gate, 404 unknown term, 409 no analyzable snapshot, 502 python-server failure |
...
- **`ai-recommendation-service`** *(src/v1/ai/service.ts)* — AI CQI recommendation pipeline behind `/api/v1/ai/recommendation/*` (frontend `server/actions/ai.ts` → dashboard `AiSuggestionsDrawer`). `generate(termId?)` resolves the term (explicit `termId` → `AiTermNotFoundError` 404 if unknown; else walks candidate terms newest-first via `startDate desc nulls last` until one has an analyzable rollup snapshot), replays each section's persisted ETL snapshot (`ComputationRun.etlSnapshotJson` — header + attainments + clo_plo_mapping; workbook header wins over DB values, same as rollup) through `compute.ts#buildSubmission` into an `InstitutionalSummaryPayload`, calls python-server `institutionalSummary()` (injectable fetcher for tests), computes worst-CLO rows against the 70% floor (`worstFromAnalyticsSummary`), and persists an `ai_recommendation` row (`recommendationText` = LLM Markdown output, `summary` = worst-CLO display rows, `sourceDataSnapshot` = full rollup + prompt, status `pending_review`). `latest()` returns the newest row across terms (or `null`) without `cached()` (freshness after generate). Errors: `AiNoDataError` (409 — no analyzable snapshots anywhere), `PythonServerError` from `lib/ingest/ingest-client` (502); the generate route additionally runs `assertCanGenerateAiInsights` (403). Pure helpers + payload mapping live in `compute.ts`, Elysia schemas in `model.ts`.
