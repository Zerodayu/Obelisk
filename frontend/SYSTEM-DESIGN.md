...
- **`server/actions/`** — all `"use server"` Server Actions, one file per domain: `auth.ts`, `car.ts`, `rollup.ts`, `cqi.ts`, `plan.ts`, `check.ts`, `academic.ts`, `forms.ts`. They are the frontend's mutation/read layer: each action authenticates/authorizes, calls `actionApi`, and returns a serializable `ActionResult` (`{ ok: true, data } | { ok: false, error }`); success navigations use `redirect()`. Client components import these actions — mutations never call `api.post` directly.
...
- `components/dashboard/` — `role-dashboard-shell` (`DashboardShell`, `StatCard`, `PendingSection`), `ai-suggestions-drawer`, plus the per-role dashboards under `app/(app)/dashboard/`.
