...
function loopStatusBadge(status: string) {
  ...
  if (status === "open_reassess")
    return <Badge variant="warning">OPEN — Re-assess</Badge>;
      ...
      const result = await saveCtl(payload.formSubmissionId, {
        rows: rows.map((r) => ({
          ...
          interventionImplementedText:
            r.interventionImplementedText || undefined,
        ...
        identify: {
          c1PriorCycleKpisAchieved:
            identify.c1PriorCycleKpisAchieved || undefined,
          c2PreviouslyMetDeclining:
            identify.c2PreviouslyMetDeclining || undefined,
          c3ExternalShifts: identify.c3ExternalShifts || undefined,
          c4ProactiveImprovements:
            identify.c4ProactiveImprovements || undefined,
        },
      });
      if (result.ok) {
    ...
    return (
      <Frame>
        <FrameHeader>
          ...
          <FrameDescription>
            Evaluate whether CQI interventions closed the attainment gap. Loop
            status is computed from 5 conditions.
          </FrameDescription>
        </FrameHeader>
        <FramePanel>
    ...
    <div className="space-y-4">
      <Frame>
        <FrameHeader>
          ...
          <FrameDescription>
            {payload.term.schoolYear} {payload.term.semester} · {rows.length}{" "}
            CQI entries
          </FrameDescription>
        </FrameHeader>
        <FramePanel>
              ...
              <tbody>
                {rows.map((row, idx) => (
                  <tr key={row.id} className="border-b last:border-0">
                    ...
                    {(
                      ...
                      <td key={field} className="py-1 pr-2 text-center">
                        <input
                          ...
                          checked={row[field]}
                          onChange={(e) =>
                            updateRow(idx, field, e.target.checked)
                          }
                    ...
                    <td className="py-1 pr-2">
                      {loopStatusBadge(row.loopStatus)}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <FramePanel>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field>
              ...
              <Textarea
                value={identify.c1PriorCycleKpisAchieved || ""}
                onChange={(e) =>
                  setIdentify({
                    ...identify,
                    c1PriorCycleKpisAchieved: e.target.value,
                  })
                }
                rows={3}
              />
            ...
            <Field>
              ...
              <Textarea
                value={identify.c2PreviouslyMetDeclining || ""}
                onChange={(e) =>
                  setIdentify({
                    ...identify,
                    c2PreviouslyMetDeclining: e.target.value,
                  })
                }
                rows={3}
              />
            ...
            <Field>
              ...
              <Textarea
                value={identify.c3ExternalShifts || ""}
                onChange={(e) =>
                  setIdentify({ ...identify, c3ExternalShifts: e.target.value })
                }
                rows={3}
              />
            </Field>
            <Field>
              ...
              <Textarea
                value={identify.c4ProactiveImprovements || ""}
                onChange={(e) =>
                  setIdentify({
                    ...identify,
                    c4ProactiveImprovements: e.target.value,
                  })
                }
                rows={3}
              />
