...
interface ToastErrorOptions {
  title: string;
  description?: string;
  /** HTTP status code when known (`ApiError.status`). */
  status?: number;
  /** Fallback dedupe key used when there's no HTTP status. */
  scope: string;
}

/**
 * Error toast that persists until dismissed and dedupes by HTTP status code, so
 * the same error re-firing while visible replaces itself instead of stacking.
 */
