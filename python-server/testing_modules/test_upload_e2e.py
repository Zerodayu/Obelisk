...
def main():
    """Runs the end-to-end test for file upload and processing with AUN-OBE template."""
    ...
    try:
        ...
        # Verify excluded_reason is None for all records (CLO-PLO mapping retired)
        for record in attainments:
            assert record.get("excluded_reason") is None, f"Expected excluded_reason=None, got {record.get('excluded_reason')}"
            assert "indirect_clo_attainment_pct" in record, "Missing indirect_clo_attainment_pct in record"

        print(f"\u2713 All {num_attainments} records have excluded_reason=None and valid indirect_clo_attainment_pct.")
        ...
        try:
            rec_response = requests.get(recommendation_url, timeout=20)
        ...
        print(f"Error during E2E test: {exc}")
        print("\nEND-TO-END TEST: FAILED")
